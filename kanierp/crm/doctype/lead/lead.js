...
kanierp.LeadController = class LeadController extends frappe.ui.form.Controller {
	setup() {
	...
	add_lead_to_prospect(frm) {
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "kanierp.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "kanierp.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		frappe.model.open_mapped_doc({
			method: "kanierp.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
...
extend_cscript(cur_frm.cscript, new kanierp.LeadController({ frm: cur_frm }));
