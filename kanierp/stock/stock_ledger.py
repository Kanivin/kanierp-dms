...
import kanierp
from kanierp.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from kanierp.stock.utils import (
...
from kanierp.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from kanierp.controllers.stock_controller import future_sle_exists
...
class update_entries_after:
	...
	def get_serialized_values(self, sle):
		from kanierp.stock.serial_batch_bundle import SerialNoValuation
	...
	def update_serial_no_status(self, sle):
		from kanierp.stock.serial_batch_bundle import get_serial_nos
	...
	def update_batched_values(self, sle):
		from kanierp.stock.serial_batch_bundle import BatchNoValuation
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=kanierp.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(kanierp.is_perpetual_inventory_enabled(company))
	):
