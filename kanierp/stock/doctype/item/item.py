...
import kanierp
from kanierp.controllers.item_variant import (
...
from kanierp.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from kanierp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from kanierp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from kanierp.stock.doctype.item_default.item_default import ItemDefault
		from kanierp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from kanierp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from kanierp.stock.doctype.item_tax.item_tax import ItemTax
		from kanierp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from kanierp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_kanierp_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from kanierp.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"kanierp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_kanierp_to_barcodenumber(kanierp_number, barcode):
	if kanierp_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if kanierp_number in convert:
		return convert[kanierp_number]
...
def get_asset_naming_series():
	from kanierp.assets.doctype.asset.asset import get_asset_naming_series
