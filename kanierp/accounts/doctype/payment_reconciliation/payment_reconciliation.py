...
from kanierp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation import (
	is_any_doc_running,
)
...
from kanierp.controllers.accounts_controller import get_advance_payment_entries_for_regional
...
class PaymentReconciliation(Document):
	...
	if TYPE_CHECKING:
		...
		from kanierp.accounts.doctype.payment_reconciliation_allocation.payment_reconciliation_allocation import (
			PaymentReconciliationAllocation,
		)
		from kanierp.accounts.doctype.payment_reconciliation_invoice.payment_reconciliation_invoice import (
			PaymentReconciliationInvoice,
		)
		from kanierp.accounts.doctype.payment_reconciliation_payment.payment_reconciliation_payment import (
	...
	def get_jv_entries(self):
		...
		account_type = kanierp.get_party_account_type(self.party_type)
	...
	def get_dr_or_cr_notes(self):
		...
		if kanierp.get_party_account_type(self.party_type) == "Receivable":
				...
				"accounts": [
					{
						...
						"cost_center": inv.cost_center or kanierp.get_default_cost_center(company),
