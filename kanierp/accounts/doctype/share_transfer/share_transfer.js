...
frappe.provide("kanierp.share_transfer");
...
frappe.ui.form.on("Share Transfer", {
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__("Create Journal Entry"), function () {
				kanierp.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null) {
			kanierp.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
...
kanierp.share_transfer.update_amount = function (frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

kanierp.share_transfer.make_jv = function (frm) {
	var account,
		payment_account,
	...
	frappe.call({
		...
		method: "kanierp.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {
