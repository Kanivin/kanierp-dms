...
frappe.provide("kanierp.accounts");

cur_frm.cscript.tax_table = "Sales Taxes and Charges";

kanierp.accounts.taxes.setup_tax_validations("Sales Invoice");
kanierp.accounts.payment_triggers.setup("Sales Invoice");
kanierp.accounts.pos.setup("Sales Invoice");
kanierp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
kanierp.sales_common.setup_selling_controller();
kanierp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	kanierp.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		kanierp.queries.setup_queries(this.frm, "Warehouse", function () {
			return kanierp.queries.warehouse(me.frm.doc);
		});
		...
		kanierp.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		kanierp.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);

		if (doc.update_stock) {
			this.show_stock_ledger();
			kanierp.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
		}
		...
		kanierp.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	}

	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm,
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				kanierp.utils.map_current_doc({
					method: "kanierp.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				kanierp.utils.map_current_doc({
					method: "kanierp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				kanierp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "kanierp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
				});
	...
	customer() {
		...
		kanierp.utils.get_party_details(
			this.frm,
			"kanierp.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: kanierp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new kanierp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "kanierp.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};
...
cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	kanierp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	kanierp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	kanierp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function () {
			return {
				query: "kanierp.controllers.queries.get_income_account",
				filters: {
					company: frm.doc.company,
					disabled: 0,
				},
		...
		(frm.custom_make_buttons = {
			...
			(frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function (doc, cdt, cdn) {
				return {
					query: "kanierp.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "kanierp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	company: function (frm) {
		if (frm.doc.company) {
			frappe.call({
				method: "kanierp.setup.doctype.company.company.get_default_company_address",
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "kanierp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "kanierp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
					customer: frm.doc.customer,
					loyalty_program: frm.doc.loyalty_program,
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "kanierp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "kanierp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
