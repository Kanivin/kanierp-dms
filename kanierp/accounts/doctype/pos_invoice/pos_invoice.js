...
kanierp.accounts.pos.setup("POS Invoice");
kanierp.selling.POSInvoiceController = class POSInvoiceController extends kanierp.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "kanierp.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
	...
	pos_profile(frm) {
		...
		frappe.call({
			method: "kanierp.selling.page.point_of_sale.point_of_sale.get_pos_profile_data",
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "kanierp.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: cur_frm,
		});
	}
};

extend_cscript(cur_frm.cscript, new kanierp.selling.POSInvoiceController({ frm: cur_frm }));
