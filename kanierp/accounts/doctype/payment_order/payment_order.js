...
frappe.ui.form.on("Payment Order", {
	...
	get_from_payment_entry: function (frm) {
		...
		kanierp.utils.map_current_doc({
			method: "kanierp.accounts.doctype.payment_entry.payment_entry.make_payment_order",
	...
	get_from_payment_request: function (frm) {
		frm.trigger("remove_row_if_empty");
		kanierp.utils.map_current_doc({
			method: "kanierp.accounts.doctype.payment_request.payment_request.make_payment_order",
	...
	make_payment_records: function (frm) {
			...
			fields: [
				{
					...
					get_query: function () {
						return {
							query: "kanierp.accounts.doctype.payment_order.payment_order.get_supplier_query",
...

				{
					...
					get_query: function () {
						return {
							query: "kanierp.accounts.doctype.payment_order.payment_order.get_mop_query",
							filters: { parent: frm.doc.name },
						};
					},
				},
		...
		dialog.set_primary_action(__("Submit"), function () {
			...
			return frappe.call({
				method: "kanierp.accounts.doctype.payment_order.payment_order.make_payment_records",
				args: {
					name: frm.doc.name,
