...
frappe.provide("kanierp.queries");
$.extend(kanierp.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "kanierp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "kanierp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "kanierp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "kanierp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "kanierp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
kanierp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
kanierp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = kanierp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "kanierp.controllers.queries.warehouse_query" });
