...
kanierp.buying = {
	setup_buying_controller: function() {
		kanierp.buying.BuyingController = class BuyingController extends kanierp.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return kanierp.queries.company_address_query(this.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', kanierp.queries.supplier);
				me.frm.set_query('contact_person', kanierp.queries.contact_query);
				me.frm.set_query('supplier_address', kanierp.queries.address_query);

				me.frm.set_query('billing_address', kanierp.queries.company_address_query);
				kanierp.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "kanierp.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				kanierp.utils.get_party_details(this.frm, null, null, function(){
					me.apply_price_list();
				});
			}
			...
			company(){
				...
				frappe.call({
					method: "kanierp.setup.doctype.company.company.get_billing_shipping_address",
					args: {
			...
			supplier_address() {
				kanierp.utils.get_address_display(this.frm);
				kanierp.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			warehouse(doc, cdt, cdn) {
				...
				if(item.item_code && item.warehouse) {
					return this.frm.call({
						method: "kanierp.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					kanierp.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				kanierp.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				kanierp.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "kanierp.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new kanierp.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
...
kanierp.buying.link_to_mrs = function(frm) {
	frappe.call({
...
kanierp.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "kanierp.stock.get_item_details.get_default_bom",
				args: {
...
kanierp.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "kanierp.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
