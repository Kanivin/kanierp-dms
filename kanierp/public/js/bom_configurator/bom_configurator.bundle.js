class BOMConfigurator {
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "kanierp.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	add_item(node, view) {
		frappe.prompt(
			...
			(data) => {
				...
				frappe.call({
					method: "kanierp.manufacturing.doctype.bom_creator.bom_creator.add_item",
					args: {
	...
	add_sub_assembly(node, view) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			frappe.call({
				method: "kanierp.manufacturing.doctype.bom_creator.bom_creator.add_sub_assembly",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
					fg_reference_id: node.data.name || this.frm.doc.name,
	...
	delete_node(node, view) {
		frappe.confirm(__("Are you sure you want to delete this Item?"), () => {
			frappe.call({
				method: "kanierp.manufacturing.doctype.bom_creator.bom_creator.delete_node",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
	...
	edit_qty(node, view) {
		...
		frappe.prompt(
			...
			(data) => {
				...
				frappe.call({
					method: "kanierp.manufacturing.doctype.bom_creator.bom_creator.edit_qty",
					args: {
						doctype: doctype,
						docname: docname,
						qty: data.qty,
