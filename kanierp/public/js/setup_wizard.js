...
frappe.setup.on("before_load", function () {
	kanierp.setup.slides_settings.map(frappe.setup.add_slide);
});

kanierp.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = kanierp.setup.fiscal_years[country];
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "kanierp.accounts.utils.get_coa",
				args: {
