frappe.provide("kanierp.financial_statements");

kanierp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		if (data && column.fieldname == "account") {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"kanierp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		kanierp.financial_statements.filters = get_filters();

		let fiscal_year = kanierp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: kanierp.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = kanierp.utils.get_fiscal_year(frappe.datetime.get_today(), false, true);
	if (fiscal_year) {
		let fy = kanierp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
