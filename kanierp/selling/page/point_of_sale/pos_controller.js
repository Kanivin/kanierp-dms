kanierp.PointOfSale.Controller = class {
	constructor(wrapper) {
	...
	fetch_opening_entry() {
		return frappe.call("kanierp.selling.page.point_of_sale.point_of_sale.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: async function ({ company, pos_profile, balance_details }) {
				...
				const method = "kanierp.selling.page.point_of_sale.point_of_sale.create_opening_voucher";
	...
	init_item_selector() {
		this.item_selector = new kanierp.PointOfSale.ItemSelector({
	...
	init_item_cart() {
		this.cart = new kanierp.PointOfSale.ItemCart({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_item_details() {
		this.item_details = new kanierp.PointOfSale.ItemDetails({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_payments() {
		this.payment = new kanierp.PointOfSale.Payment({
			wrapper: this.$components_wrapper,
			events: {
				get_frm: () => this.frm || {},
	...
	init_recent_order_list() {
		this.recent_order_list = new kanierp.PointOfSale.PastOrderList({
			wrapper: this.$components_wrapper,
			events: {
	...
	init_order_summary() {
		this.order_summary = new kanierp.PointOfSale.PastOrderSummary({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		const method = "kanierp.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos";
	...
	get_available_stock(item_code, warehouse) {
		const me = this;
		return frappe.call({
			method: "kanierp.accounts.doctype.pos_invoice.pos_invoice.get_stock_availability",
			args: {
				item_code: item_code,
				warehouse: warehouse,
			},
