...
from kanierp.accounts.party import get_dashboard_info, validate_party_accounts
from kanierp.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from kanierp.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from kanierp.accounts.doctype.party_account.party_account import PartyAccount
		from kanierp.selling.doctype.customer_credit_limit.customer_credit_limit import (
			CustomerCreditLimit,
		)
		from kanierp.selling.doctype.sales_team.sales_team import SalesTeam
		from kanierp.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def copy_communication(self):
		...
		from kanierp.crm.utils import copy_comments, link_communications
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "kanierp.selling.doctype.customer.customer.send_emails",
