...
frappe.ui.form.on("Customer", {
	setup: function (frm) {
		frm.make_methods = {
			Quotation: () =>
				frappe.model.open_mapped_doc({
					method: "kanierp.selling.doctype.customer.customer.make_quotation",
					frm: cur_frm,
				}),
			Opportunity: () =>
				frappe.model.open_mapped_doc({
					method: "kanierp.selling.doctype.customer.customer.make_opportunity",
					frm: cur_frm,
				}),
		};
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "kanierp.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					customer: doc.name,
				},
			};
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(
				__("Pricing Rule"),
				function () {
					kanierp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
			...
			// indicator
			kanierp.utils.set_party_dashboard_indicators(frm);
		} else {
	...
	show_party_link_dialog: function (frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				frappe.call({
					method: "kanierp.accounts.doctype.party_link.party_link.create_party_link",
					args: {
