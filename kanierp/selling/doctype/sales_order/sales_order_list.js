frappe.listview_settings["Sales Order"] = {
	...
	onload: function (listview) {
		var method = "kanierp.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		if (frappe.model.can_create("Sales Invoice")) {
			listview.page.add_action_item(__("Sales Invoice"), () => {
				kanierp.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
			});
		}

		if (frappe.model.can_create("Delivery Note")) {
			listview.page.add_action_item(__("Delivery Note"), () => {
				kanierp.bulk_transaction_processing.create(listview, "Sales Order", "Delivery Note");
			});
		}

		if (frappe.model.can_create("Payment Entry")) {
			listview.page.add_action_item(__("Advance Payment"), () => {
				kanierp.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
			});
		}
	},
};
