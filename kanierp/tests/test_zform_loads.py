...
class TestFormLoads(KanivinTestCase):
	@change_settings("Print Settings", {"allow_print_for_cancelled": 1})
	def test_load(self):
		kanierp_modules = frappe.get_all("Module Def", filters={"app_name": "kanierp"}, pluck="name")
		doctypes = frappe.get_all(
			"DocType",
			{"istable": 0, "issingle": 0, "is_virtual": 0, "module": ("in", kanierp_modules)},
			pluck="name",
		)
