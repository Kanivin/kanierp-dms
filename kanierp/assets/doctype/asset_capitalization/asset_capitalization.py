...
import kanierp
from kanierp.assets.doctype.asset.asset import get_asset_value_after_depreciation
...
from kanierp.stock.utils import get_incoming_rate
...
class AssetCapitalization(StockController):
	...
	if TYPE_CHECKING:
		...
		from kanierp.assets.doctype.asset_capitalization_asset_item.asset_capitalization_asset_item import (
			AssetCapitalizationAssetItem,
		)
		from kanierp.assets.doctype.asset_capitalization_service_item.asset_capitalization_service_item import (
			AssetCapitalizationServiceItem,
		)
		from kanierp.assets.doctype.asset_capitalization_stock_item.asset_capitalization_stock_item import (
	...
	def validate_item(self, item):
		from kanierp.stock.doctype.item.item import validate_end_of_life
		...
		for item_row in self.stock_items:
			...
			if sle_list:
				for sle in sle_list:
					...
					if kanierp.is_perpetual_inventory_enabled(self.company):
